...
namespace Tests.ServisiTests
{
    ...
    public class ProizvodnjaVinaServisTests
    {
        ...
        {
            var obranaLoza = new Loza("Sangiovese", 23.0, 2024, "Chianti") { Id = 1, FazaZrelosti = FazaZrelosti.Obrana };
            mockLozeRepo.Setup(r => r.PronadjiLozePoFaziZrelosti(FazaZrelosti.Obrana))
                .Returns(new List<Loza> { obranaLoza });
            mockVinaRepo.Setup(r => r.DodajVino(It.IsAny<Vino>())).Returns((Vino v) => { v.Id = 1; return v; });
        ...
        {
            var novaLoza = new Loza("TestVino", 20.0, 2025, "Toskana") { Id = 2, FazaZrelosti = FazaZrelosti.Posadjena };
